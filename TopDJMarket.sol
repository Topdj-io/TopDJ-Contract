...
contract MarketplaceStorage {
  ...
  ERC20Interface public payToken;
...
contract TopDJMarketplace is Initializable, Ownable, Pausable, MarketplaceStorage, NativeMetaTransaction {
  ...
  )initializer
  {
    ...
    // EIP712 init
    _initializeEIP712('TopDJ Marketplace', '1');

    // Fee init
    setTradeFee(_tradeFee);

    setTeamAddress(_teamAddress);
    payToken = ERC20Interface(_payTokenAddress);
  ...
  function setPublicationFee(uint256 _publicationFee) external onlyOwner {
    require(_publicationFee < 100 ether, "Invalid publicationFee");
    ...
    internal
  {
    ...
    if (publicationFeeInWei > 0) {
      require(
        payToken.transferFrom(sender, teamAddress, publicationFeeInWei),
  ...
  {    
    ...
    require(
    payToken.transferFrom(sender, teamAddress, fee),
    "_transferMoney::Transfering the cut to the Marketplace owner failed"
    );

    // Transfer sale amount to seller
    require(
    payToken.transferFrom(sender, seller, price.sub(fee)),
